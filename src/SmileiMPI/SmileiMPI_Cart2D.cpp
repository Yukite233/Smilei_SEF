...
void SmileiMPI_Cart2D::createTopology(PicParams& params)
{
    ...
    MPI_Allreduce(dims_gather_temp, dims_gather, 2*smilei_sz, MPI_INT,MPI_SUM, SMILEI_COMM_2D);
...
void SmileiMPI_Cart2D::scatterGrid( Grid* grid )
{
    int procs_rk;
    int iGlobal, jGlobal;
    int iGlobal_gather;
            ...
            for(int i = 0; i < dims_gather[procs_rk*2]; i++)
            {
                for(int j = 0; j < dims_gather[procs_rk*2 + 1]; j++)
                {
                    ...
                    if(iProcs == 0 && i < oversize[0] || iProcs == number_of_procs[0] -1 && i > dims_gather[procs_rk*2] - 1 - oversize[0]){
                        iGlobal = abs((int)grid2d->globalDims_[0] - abs(iGlobal) - 1);
                    }
                    if(jProcs == 0 && j < oversize[1] || jProcs == number_of_procs[1] -1 && j > dims_gather[procs_rk*2+1] - 1 - oversize[1]){
                        jGlobal = abs((int)grid2d->globalDims_[1] - abs(jGlobal) - 1);
                    }
                    ...
                    grid_global_gather[iGlobal_gather] = grid2d->iswall_global_2D[iGlobal][jGlobal];
    ...
    MPI_Scatterv(grid_global_gather, &send_cnt[0], &send_disp[0], MPI_INT, &grid2d->iswall_2D[0][0], recv_cnt[smilei_rk], MPI_INT, 0, SMILEI_COMM_2D);
    //TITLE("scatterGrid");
...
void SmileiMPI_Cart2D::scatterField( Field* field_global ,Field* field )
{
    ...
    int ii,jj;
    ii=f2D_global->dims_[0];
    jj=f2D_global->dims_[1];
        ...
        for(int jProcs = 0; jProcs < number_of_procs[1]; jProcs++)
        {
            ...
            for(int i = 0; i < dims_gather[procs_rk*2]; i++)
            {
                for(int j = 0; j < dims_gather[procs_rk*2 + 1]; j++)
                {
                    ...
                    field_global_gather[iGlobal_gather] = f2D_global->data_2D[iGlobal][jGlobal];
    ...
    MPI_Scatterv(field_global_gather, &send_cnt[0], &send_disp[0], MPI_DOUBLE, f2D->data_, recv_cnt[smilei_rk], MPI_DOUBLE, 0, SMILEI_COMM_2D);

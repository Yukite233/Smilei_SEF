...
int main (int argc, char* argv[])
{
    ...
    TITLE("Initializing particles, fields and fieldsolver");
    ...
    for (unsigned int itime=stepStart+1 ; itime <= stepStop ; itime++) {
        ...
        int tid(0);
        (EMfields->rho_)->put_to(0.0);
        for (unsigned int ispec=0 ; ispec<params.species_param.size(); ispec++) {
            if (1 ){
                (EMfields->rho_s[ispec])->put_to(0.0);
        ...
        if(smpi->isMaster() && itime % params.dump_step == 0){
            sio->write(EMfields, smpi);
        }
        smpi->barrier();
